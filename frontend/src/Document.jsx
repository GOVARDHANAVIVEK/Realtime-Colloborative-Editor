...
const Document = ({doc,userId}) => {
    ...
    //         const response = await fetch(`http://localhost:3900/api/documents/${docid}`, {
    //             method: "DELETE",
    //             headers: {
    ...
    //         // If response contains JSON, parse it
    //         const data = await response.json().catch(() => null);
    
    //         alert(data?.message || "Document deleted successfully");
    //         socket.emit('documentDeleted',data?.result)
    //     } catch (error) {
    ...
    useEffect(()=>{
        console.log("TrackVersions",checkedDocs)
    },[checkedDocs])
    ...
    return (
        ...
        <div className="w-56 h-72 bg-white border border-gray-300 rounded-lg shadow-md flex flex-col overflow-hidden">
            {/* Header Section */}
            <div className="w-full bg-yellow-600 text-white text-center py-2 font-semibold flex flex-row  px-2">
                               
                {/* Options Menu */}
                <button 
                    className="w-[15%] text-white text-lg ml-auto" 
               ...
               <div className="flex flex-col space-y-3">
                   {doc.owner === userId && (
                       <button 
                           className="w-full px-4 py-2 text-red-600 font-medium bg-red-50 rounded-md hover:bg-red-100 transition"
                   ...
                   <button 
                       className="w-full px-4 py-2 text-blue-600 font-medium bg-blue-50 rounded-md hover:bg-blue-100 transition"
                       onClick={() => onShare(doc._id)}
                   >
                       🔗 Share Access
                   </button>
                   <button 
                       className="w-full px-4 py-2 text-black font-medium bg-green-50 rounded-md hover:bg-green-100 transition"
            ...
            <div className="border-1 border-gray-200 py-2 text-center w-full bg-blue-600 rounded-lg font-semibold">
                <button className="text-white hover:underline" onClick={() => navigate(`/document/editor/${doc._id}`)}>Open</button>
            </div>
        </div>
